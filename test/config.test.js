...
test('rejeita cena com ID perigoso', () => {
  const value = { configVersion: 7, api: { host: '127.0.0.1', port: 47831, token: '' }, scenes: [{ id: '../x', name: 'Inválida', color: '#ffffff', brightness: 1 }], controllers: [] };
...
test('adiciona controladores de iluminação ambiente ao migrar da versão 6', () => {
  const directory = fs.mkdtempSync(path.join(os.tmpdir(), 'rgb-central-ambient-v7-'));
  const configPath = path.join(directory, 'config.json');
  const oldConfig = {
    configVersion: 6,
    api: { host: '127.0.0.1', port: 47831, token: 'token-local' },
    scenes: [{ id: 'green', name: 'Verde', color: '#00ff00', brightness: 70 }],
    controllers: [{
      id: 'simulation', name: 'Demonstração', type: 'simulation',
      configured: true, enabled: true, ignored: false
    }]
  };
  fs.writeFileSync(configPath, JSON.stringify(oldConfig), 'utf8');
  try {
    ...
    const migrated = loadConfig(configPath);
    assert.equal(migrated.configVersion, 7);
    assert.deepEqual(migrated.controllers.map((controller) => controller.id), ['simulation', 'govee', 'ambient']);
    assert.equal(migrated.controllers[1].configured, false);
    assert.equal(migrated.controllers[2].token, '');
  } finally {
    fs.rmSync(directory, { recursive: true, force: true });
  }
});

test('valida Govee e Home Assistant somente depois da configuração completa', () => {
  const value = JSON.parse(fs.readFileSync(path.join(__dirname, '..', 'config', 'default.json'), 'utf8'));
  const govee = value.controllers.find((controller) => controller.id === 'govee');
  govee.configured = true;
  assert.throws(() => validateConfig(value), /Nenhum dispositivo Govee/);
  govee.configured = false;
  const ambient = value.controllers.find((controller) => controller.id === 'ambient');
  ambient.configured = true;
  assert.throws(() => validateConfig(value), /Home Assistant incompleta/);
});
